...
def make_glookup(class_assignments):
    ...
    getpoint = 0
    sumpoint = 0
    def calculate(message,point):
        nonlocal getpoint,sumpoint
        getpoint += point
        sumpoint += class_assignments[message]
        return getpoint/sumpoint
    return calculate
...
def make_advanced_counter_maker():
    ...
    1
    """
    g = {"global-count":0,'count':[]}
    def make_counter(num = [0]):
        no,num[0] = num[0],num[0]+1
        def counter(mse):
            nonlocal g
            "*** YOUR CODE HERE ***"
            if mse == 'count':
